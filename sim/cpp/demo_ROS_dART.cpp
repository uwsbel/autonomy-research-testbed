...
//     std::string directoryPath = "/home/art/art/sim/data_art/autonomy-toolkit/paths/";
...
int main(int argc, char* argv[]) {
    ...
    vehicle.SetChassisFixed(false);
    vehicle.SetInitPosition(ChCoordsys<>(initLoc, initRot));
    vehicle.SetTireType(TireModelType::TMEASY);
    ...
    ChFrame<double> sensor_offset_pose(ChVector<>(0, 0, 1), Q_from_AngAxis(0, {1, 0, 0}));
    ...
    // Add a lidar sensor
//     auto lidar_offset_pose = chrono::ChFrame<double>({0, 0, 0.4}, QUNIT);
//     auto lidar = chrono_types::make_shared<ChLidarSensor>(vehicle.GetChassis()->GetBody(), 10, lidar_offset_pose, 900, 30, CH_C_PI, 0, 0, 100.0f);
//     lidar->PushFilter(chrono_types::make_shared<ChFilterVisualize>(640, 480, "Lidar Visualize"));
//     manager->AddSensor(lidar);
...
// //     manager->AddSensor(lidar);
    ...
    // Simulation loop
    double step_size = 1e-3;
    double time_end = 30; // 30 seconds simulation
    double throttle = 0.5; // Constant throttle value

...
int main(int argc, char* argv[]) {
    ...
    auto mmesh = ChTriangleMeshConnected::CreateFromWavefrontFile(GetChronoDataFile("vehicle/audi/audi_chassis.obj"),
                                                                  false, true);
    mmesh->Transform(ChVector3<double>(0., 0., 0.), ChMatrix33<>(1));
    ...
    mesh_body->AddVisualShape(trimesh_shape, ChFrame<>(ChVector3<double>(0., 0., 0.)));
    ...
    quat.SetFromAngleAxis(0.2, ChVector3<double>(0,1,0));
    chrono::ChFrame<double> offset_pose({-8, 0, 2}, quat);
    ...
    auto cam = chrono_types::make_shared<ChCameraSensor>(ground_body, 30, offset_pose, 1280, 720, 3.14159 / 3.);
    cam->PushFilter(chrono_types::make_shared<ChFilterRGBA8Access>());
    cam->PushFilter(chrono_types::make_shared<ChFilterVisualize>(1280, 720));
    sensor_manager->AddSensor(cam);

    // Create a lidar and add it to the sensor manager
    auto lidar = chrono_types::make_shared<ChLidarSensor>(ground_body, 5.f, offset_pose, 900, 30, 2 * 3.14159,
                                                          3.14159 / 12, -3.14159 / 6, 100.0f);
    ...
    auto lidar_2d =
        chrono_types::make_shared<ChLidarSensor>(ground_body, 5.f, offset_pose, 480, 1, 2 * 3.14159, 0.0, 0.0, 100.0f);
    ...
    ChVector3<double> gps_reference(-89.400, 43.070, 260.0);

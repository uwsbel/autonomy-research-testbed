...
class ART1(veh.RCCar):
    def __init__(self, init_loc: chrono.ChVectorD, init_rot: chrono.ChQuaternionD):
        ...
        self.cam = sens.Sensor(sensor_path / "camera.json", chassis_body, camera_pose)

        offset_pose = chrono.ChFrameD()
        self.acc = sens.Sensor(sensor_path / "accelerometer.json", chassis_body, offset_pose)
        self.gyro = sens.Sensor(sensor_path / "gyroscope.json", chassis_body, offset_pose)
        self.mag = sens.Sensor(sensor_path / "magnetometer.json", chassis_body, offset_pose)
        self.gps = sens.Sensor(sensor_path / "gps.json", chassis_body, offset_pose)

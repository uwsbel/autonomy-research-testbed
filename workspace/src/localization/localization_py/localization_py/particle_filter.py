...
class ParticleFilter:
    ...
    def update(self, u, obs):
        ...
        for i in range(0, self.num_particles):
            myu = np.array(
                [
                    [u[0, 0] + np.random.normal(0, 0.05)],
                    [u[1, 0] + np.random.normal(0, 0.05)],
                ]
            )
        ...
        for i in range(0, self.num_particles):
            for j in range(0, 4):
                state[j, 0] = (
                    state[j, 0] + self.particles[i][j, 0] * self.particle_weights[i]
                )
    ...
    def update_dist(self, obs):
        ...
        for i in range(0, self.num_particles):
            distance = math.sqrt(
                (self.particles[i][0, 0] - obs[0, 0]) ** 2
                + (self.particles[i][1, 0] - obs[1, 0]) ** 2
            )
    ...
    def resample(self):
        ...
        cumulative_weights = [
            sum(self.particle_weights[: i + 1]) for i in range(self.num_particles)
        ]
        sampled_points = []
    ...
    def assign_weights(self):
        ...
        """
        for i in range(0, self.num_particles):
            self.particle_weights[i] = 0.9 / wasserstein_distance(
                self.particle_distr_dist[i], self.dist_distr
            ) + 0.1 / wasserstein_distance(self.particle_distr_head[i], self.head_distr)

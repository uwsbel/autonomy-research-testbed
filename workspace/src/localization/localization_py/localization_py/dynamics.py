...
class Dynamics:
    ...
    def motion_model(self, x, u):
        ...
        f = self.tau_0 * u[0, 0] - self.tau_0 * x[3, 0] / (
            self.omega_0 * self.r_wheel * self.gamma
        )

        x[3, 0] = x[3, 0] + self.dt * ((self.r_wheel * self.gamma) / self.i_wheel) * (
            f - (x[3, 0] * self.c_1) / (self.r_wheel * self.gamma) - self.c_0
        )
    ...
    def calc_F(self, v, theta, delta):
        ...
        F = np.array(
            [
                ...
                [0.0, 1.0, self.dt * v * math.cos(theta), self.dt * math.sin(theta)],
                [0.0, 0.0, self.dt, self.dt * np.tan(delta) / self.l],
                [
                    0.0,
                    0.0,
                    0.0,
                    ...
                    * (
                        1
                        + (self.r_wheel * self.gamma / (self.i_wheel))
                        * (
                            -(self.tau_0 / (self.omega_0 * self.r_wheel * self.gamma))
                            - (self.c_1 / (self.r_wheel * self.gamma))
                        )
                    ),
                ],
            ]

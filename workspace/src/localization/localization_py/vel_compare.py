...
class GPSVelocitySubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        velocity_x = abs(msg.velocity.x)  # Extracting the x-component of velocity
        #print(velocity_x)
...
class OdomSubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        velocity_y = abs(msg.twist.twist.linear.y)
...
#     gps_times = [(t[0] - start_time) for t in gps_velocity_subscriber.gps_velocity_data]
    ...
    #plt.plot([d[0] for d in odom_subscriber.odom_data], [d[1] for d in odom_subscriber.odom_data], label='State-Estimator Velocity (x)')
    plt.plot([d[0] for d in odom_subscriber.odom_data], [d[2] for d in odom_subscriber.odom_data], label='State-Estimator Velocity')
...
#     # Integrate the IMU acceleration data to get velocity
#     #imu_velocity = integrate_acceleration(imu_subscriber.imu_data)

#     # # Save IMU velocity data to CSV
#     # save_data_to_csv('imu_velocity.csv', list(zip([d[0] for d in imu_subscriber.imu_data], imu_velocity)))

#     # Save Odometry data to CSV
#     save_data_to_csv('odom_data.csv', odom_subscriber.odom_data)
